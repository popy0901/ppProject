#include "Lib/Misc.h"
#include "Lib/Graphics/LambertShader.h"
#include "Lib/Graphics/DestractionShader.h"
...
namespace PP
{
	namespace Class
	{
		...
		// コンストラクタ
		Graphics::Graphics(HWND hWnd, bool isFullscreen)
		{
			// インスタンス設定
			...
			// デバイス＆スワップチェーンの生成
			{
				...
				DXGI_SWAP_CHAIN_DESC swapchainDesc;
				{
					...
					swapchainDesc.Windowed = !isFullscreen;		// ウインドウモードか、フルスクリーンにするか。
			...
			// レンダーターゲットビューの生成
			{
				...
				hr = swapchain.Get()->GetBuffer(0, __uuidof(ID3D11Texture2D), reinterpret_cast<void**>(backBuffer.GetAddressOf()));
				_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
			...
			// 深度ステンシルビューの生成
			{
				...
				::memset(&depthStencilBufferDesc, 0, sizeof(depthStencilBufferDesc));
			...
			// シェーダー
			{
				lambert = std::make_unique<LambertShader>(device.Get());
				destraction = std::make_unique<DestractionShader>(device.Get());
				renderShadow = std::make_unique<RenderShadowShader>(device.Get());
				//gaussShader = std::make_unique<GaussShader>(device.Get());
			}

			light = std::make_unique<LibLight>(); // ライト
			...
			// レンダラ
			{
				...
				primitive2d = std::make_unique<Primitive2d>(device.Get());
			}
		...
		void Graphics::ScreenClear(const DirectX::XMFLOAT4& color)
		{
		...
		void Graphics::SetDestractionParamter(float scale, float angle, float position) // 試作
		{
			DestractionShader* ds = dynamic_cast<DestractionShader*>(destraction.get());
			ds->Set(scale, angle, position);
		}

		LibShader* Graphics::GetDepthShadow() const
		{
			RenderShadowShader* rs = dynamic_cast<RenderShadowShader*>(renderShadow.get());
			return rs->GetDepthShadow();
		}

		void Graphics::SetShadowTarget(const DirectX::XMFLOAT3& target)
		{
			RenderShadowShader* rs = dynamic_cast<RenderShadowShader*>(renderShadow.get());
			rs->GetDepthShadow()->SetShadowTarget(target);
		}
