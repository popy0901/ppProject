...
namespace PP
{
	namespace Class
	{
		DestractionShader::DestractionShader(ID3D11Device* device)
		{
			// 頂点シェーダー
			{
				// ファイルを開く
				FILE* fp = nullptr;
				fopen_s(&fp, "Shader\\DestractionVS.cso", "rb");
			...
			// ピクセルシェーダー
			{
				// ファイルを開く
				FILE* fp = nullptr;
				fopen_s(&fp, "Shader\\DestractionPS.cso", "rb");
				_ASSERT_EXPR_A(fp, "CSO File not found");
			...
			// ジオメトリシェーダー
			{
				// ファイルを開く
				FILE* fp = nullptr;
				fopen_s(&fp, "Shader\\DestractionGS.cso", "rb");
				_ASSERT_EXPR_A(fp, "CSO File not found");
		...
		// 描画開始
		void DestractionShader::Begin(ID3D11DeviceContext* dc, const RenderContext& rc)
		{
		...
		// 描画
		void DestractionShader::Draw(ID3D11DeviceContext* dc, const LibModel* model)
		{
		...
		// 描画終了
		void DestractionShader::End(ID3D11DeviceContext* dc)
		{
		...
		void DestractionShader::Set(float scale, float angle, float position)
		{
			paramter.y = scale;
			paramter.z = angle;
			paramter.w = position;
